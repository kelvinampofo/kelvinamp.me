...
export default function Callout({ color, children, className, isAlert, ariaLabel }: CalloutProps) {
  const classes = clsx({
    'bg-red-100 text-red-700 border-red-200 dark:bg-red-950 dark:text-red-500 dark:border-red-900':
      color === 'red',
    'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-500 dark:border-amber-900':
      color === 'amber',
  ...
  return (
    ...
    >
      {!isAlert ? <InfoCircledIcon /> : <ExclamationTriangleIcon />}
