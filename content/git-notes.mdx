...
2. <code>git config -g branch.sort -committerdate</code> – I never liked that git branches are not
   sorted by the last commit time first. This fixes the problem.
3. <code>git config -g column.ui auto</code> – Git branches are laid out in a list by default. I
   prefer them to be displayed in columns graphically.
